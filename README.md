...
Today the pipeline already uses `AsyncOpenAI` with `await` everywhere (`ai.py`), so a single FastAPI worker can have many in-flight OpenAI calls concurrently — `_enrich_event` runs `ASSIGN_STATUS` and `EXTRACT_FEATURES` in parallel via `asyncio.gather`, and `extract_events_for_all_channels` parallelises across channels the same way. Async I/O is the right shape for inside one process. What async *doesn't* fix is everything outside one process: the user still blocks on `POST /messages` until the chain (`ROUTE_MESSAGE` → `_enrich_event` → customer rollup) completes (~1-3s tail), one uvicorn worker tops out at its event-loop / CPU ceiling, and a transient OpenAI 5xx becomes a hard request error rather than a retried operation. None of those are async-vs-sync problems — they're request-coupling, single-process, and durability problems.

The natural shape at scale is **SQS + Lambda** to break the request coupling and **SSE** to replace polling on the client side:

- **`POST /messages` becomes an enqueue.** The handler validates the payload, drops it into an SQS queue (`messages-incoming`), and returns `202 Accepted` immediately. The user-perceived latency goes from 1-3s to tens of milliseconds because the LLM chain leaves the request path entirely. A slow OpenAI tail no longer pins the worker.
- **One Lambda per pipeline step.** `route-message`, `extract-events`, `assign-status`, `extract-features` each consume their own queue. `route-message` decides attach/create/drop and either updates the event row in Postgres or publishes to `events-needing-enrichment`; the enrichment Lambdas fan out `ASSIGN_STATUS` and `EXTRACT_FEATURES` (still async / parallel within each invocation, just like today) and write results back. Each queue hop gets SQS's retry + DLQ behavior automatically — a flaky OpenAI response retries with backoff instead of losing data.
- **Horizontal scale = queue concurrency.** Lambda concurrency limits + SQS visibility timeouts give back-pressure and elasticity for free. Spike from 10 messages/min to 1000 → Lambdas scale out, queue drains, no infra change. OpenAI per-account rate limits become the actual ceiling, which is fine — at that point you split work across multiple OpenAI orgs or add a token-bucket throttler in front of the queue.
- **SSE for client fan-out.** The 5-second poll works for a handful of tabs but burns request volume linearly with users (`N clients × every-5s × every endpoint`). Replace it with **per-customer SSE channels** (`/sse/customers/{channel}/events`): the API holds a persistent connection per subscriber. When a Lambda finishes enrichment and writes Postgres, it publishes a small event (`{event_id, status, severity_label, updated_at}`) to Redis pub/sub or DynamoDB Streams; the API server subscribes and fans out only to clients currently watching that customer. Frontend swaps `<PollRefresh>` for `useEventSource(...)`; the rest of the rendering layer is unchanged because the server emits the same JSON shapes — just push instead of pull.

End-to-end at scale: **Slack adapter / live composer → API enqueues → Lambda pipeline writes Postgres → pub/sub notifies API → SSE pushes to subscribed clients.** Each hop is independently scalable, retryable, and observable.
